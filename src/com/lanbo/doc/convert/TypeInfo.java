...
public class TypeInfo implements Resolvable {
	public static final Set<String> PRIMITIVE_TYPES = Collections
			.unmodifiableSet(new HashSet<String>(Arrays.asList("boolean",
					"byte", "char", "double", "float", "int", "long", "short",
					"void")));

	public TypeInfo(boolean isPrimitive, String dimension,
			String simpleTypeName, String qualifiedTypeName, ClassInfo cl) {
		...
		if (paramStartPos > -1) {
			...
			for (int i = entryStartPos; i < paramEndPos; i++) {
				...
				if (c == ',' && bracketNesting == 0) {
					String entry = typeString.substring(entryStartPos, i)
							.trim();
			...
			TypeInfo info = new TypeInfo(typeString.substring(entryStartPos,
					paramEndPos).trim());
			generics.add(info);
	...
	public static String typeArgumentsName(ArrayList<TypeInfo> args,
	...
	private void makeHDFRecursive(Data data, String base, boolean qualified,
		...
		// .iterator().next();
		// data.setValue(base + ".link",
		// site.linkFor(mClass.htmlPage()));
	...
	public static void makeHDF(Data data, String base,
			ArrayList<TypeInfo> types, boolean qualified,
			HashSet<String> typeVariables) {
		int i = 0;
		for (TypeInfo type : types) {
			type.makeHDFRecursive(data, base + "." + i++, qualified, false,
					typeVariables);
		}
	}
	...
	public void setBounds(ArrayList<TypeInfo> superBounds,
	...
	public String toString() {
		...
		returnString += "Primitive?: " + mIsPrimitive + " TypeVariable?: "
				+ mIsTypeVariable + " Wildcard?: " + mIsWildcard
				+ " Dimension: " + mDimension + " QualifedTypeName: "
				+ mQualifiedTypeName + " SimpleTypeName: " + mSimpleTypeName;
	...
	// qualifiedClassName, resolution.getInfoBuilder());
	//
