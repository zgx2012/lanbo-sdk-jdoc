...
public class FieldInfo extends MemberInfo {
    ...
    public FieldInfo(String name, ClassInfo containingClass, ClassInfo realContainingClass,
        ...
        super(rawCommentText, name, null, containingClass, realContainingClass, isPublic,
                isProtected, isPackagePrivate, isPrivate, isFinal, isStatic, isSynthetic,
    ...
    public FieldInfo cloneForClass(ClassInfo newContainingClass) {
        return new FieldInfo(name(), newContainingClass, realContainingClass(), isPublic(),
                isProtected(), isPackagePrivate(), isPrivate(), isFinal(), isStatic(),
                isTransient(), isVolatile(), isSynthetic(), mType, getRawCommentText(),
                mConstantValue, position(), annotations());
    }
    ...
    static boolean isConstant(boolean isFinal, boolean isStatic, Object constantValue) {
        /*
         * Note: There is an ambiguity in the doc API that prevents us from
         * distinguishing a constant-null from the lack of a constant at all. We
         * err on the side of treating all null constantValues as meaning that
         * the field is not a constant, since having a static final field
         * assigned to null is both unusual and generally pretty useless.
         */
    ...
    /**
     * Returns a canonical string representation of a floating point number. The
     * representation is suitable for use as Java source code. This method also
     * addresses bug #4428022 in the Sun JDK.
     */
    ...
    public static String javaUnescapeString(String str) throws Exception {
        final int N = str.length();
        check: {
            for (int i = 0; i < N; i++) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            case CHAR4:
                ...
                if (c >= '0' && c <= '9') {
                    ...
                    throw new Exception("bad escape sequence: '" + c + "' at pos " + i + " in: \""
                            + str + "\"");
                }
                if (state == CHAR4) {
                    buf.append(escaped);
        ...
        if (state != START) {
            throw new Exception("unfinished escape sequence: " + str);
        }
        return buf.toString();
    }
    ...
    // Check the declared value with a typed comparison, not a string
    // comparison,
    ...
    private boolean valueEquals(FieldInfo other) {
        ...
        return mType.equals(other.mType) && mConstantValue.equals(other.mConstantValue);
    }
